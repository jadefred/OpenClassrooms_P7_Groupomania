...
exports.getUserInfo = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userInfo = yield pool.query('SELECT username, avatar_url, email, admin FROM users WHERE user_id = $1', [user_id]);
...
exports.modifyUserInfo = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user_id = req.params.id;
        let avatarUrl = null;
        ...
        const avatarInDB = yield pool.query('SELECT avatar_url FROM users WHERE user_id = $1', [user_id]);
        ...
        if (req.file) {
            ...
            //Update DB
            const updateUser = yield pool.query('UPDATE users SET username = $1, avatar_url = $2 WHERE user_id = $3 RETURNING *', [username, avatarUrl, user_id]);
        ...
        //no avatar is uploaded, user might or might not deleted old image
            ...
            //update DB
            const updateUser = yield pool.query('UPDATE users SET username = $1, avatar_url = $2 WHERE user_id = $3 RETURNING *', [username, image, user_id]);
            if (updateUser.rows.length === 0) {
                res.status(500).json({ error });
            }
            res.status(200).json({ message: "Successfully updated user's profile" });
...
exports.deleteUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const deleteUserComment = yield pool.query('DELETE FROM comments WHERE user_id = $1 RETURNING *', [userId]);
        ...
        if (deleteUserComment.rows.length > 0) {
            for (const i of deleteUserComment.rows) {
                yield pool.query('UPDATE posts SET commentId = array_remove(commentId, $1), totalComment = (totalComment - 1) WHERE post_id = $2;', [i.comment_id, i.post_id]);
        ...
        yield pool.query('UPDATE posts SET likeUserId = array_remove(likeUserId, $1), likes = (likes - 1) WHERE $1 = ANY(likeUserId)', [userId]);
        //get all post id that user created
        const allPostOfUser = yield pool.query('SELECT post_id FROM posts WHERE user_id = $1', [userId]);
        //if the posts have comments, delete them all
        if (allPostOfUser.rows.length > 0) {
            for (const i of allPostOfUser.rows) {
                yield pool.query('DELETE FROM comments WHERE post_id = $1 RETURNING *', [i.post_id]);
            }
        }
        //delete all posts which related to user
        const deleteAllPost = yield pool.query('DELETE FROM posts WHERE user_id = $1 RETURNING *', [userId]);
        ...
        const avatarInDB = yield pool.query('SELECT avatar_url FROM users WHERE user_id = $1', [userId]);
        //If imageUrl in DB is not null, call function to delete old image
        if (avatarInDB.rows[0].avatar_url) {
            deleteImage(avatarInDB.rows[0].avatar_url.split('/').pop());
        }
        //delete user account
        yield pool.query('DELETE FROM users WHERE user_id = $1 RETURNING *', [
            userId,
        ]);
